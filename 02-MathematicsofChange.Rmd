...
Y <- growth_ac(N=1000,r=1.5,type = "driving")
Y1 <- as.numeric(Y/max(Y))
...
So if we have a set of timeseries as in the previous examples, we need to change it to long format.
...
# A wide data frame
df.wide <- data.frame(Ynrm = Y^2,
                      Ycum = cumsum(Y), 
                      Yuni = cumsum(Y-median(Y)),
                      t    = seq_along(Y)
                      )
...
# A long dataframe
df.long <- df.wide %>% gather(key=TimeSeries,value=Y,-t)

glimpse(df.long)

ggplot(df.long, aes(x=t, y=Y, colour=TimeSeries)) +
  geom_line() + 
  theme_bw()


ggplot(df.long, aes(x=t,y=Y)) +
  geom_line() + 
...
We can pas a vector of time points to create the exact solution, the same we would get if we were to iterate the differential/difference equation.
```{r, echo=TRUE, include=TRUE}
